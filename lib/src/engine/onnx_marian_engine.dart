...
/// ONNX Runtime backend for Marian / OPUS-MT encoder-decoder models.
///
/// The model bundle is a two-session split:
///
/// * `encoder.onnx` maps `input_ids` + `attention_mask` to `last_hidden_state`.
/// * `decoder.onnx` is optimum's *merged* decoder: one graph serving both the
///   first step (`use_cache_branch = false`) and every cached step
///   (`use_cache_branch = true`). It carries an extra `next_token` output that
///   performs the greedy arg-max inside the graph, so the 59 514-wide logits
///   tensor never crosses the FFI boundary.
///
/// This engine owns the sessions and a [MarianRunner] on the isolate that
/// called [load]; that runner serves [generate], and therefore
/// `translateSync()`. It also lazily spawns a [TranslationWorker] that attaches
/// to the *same* sessions from a background isolate, which is what serves
/// [generateChunks] and therefore `translate()` and `translateStream()`. The
/// model is loaded once either way.
class OnnxMarianEngine implements TranslationEngine {
  /// Creates an engine for an installed [model].
  OnnxMarianEngine(this.model, {this.runtimeConfig = const RuntimeConfig()});
  ...
  Future<void> load(
    ...
    try {
      final tokenizer = MarianTokenizer.fromAssets(
        ...
        vocabJson: await File(p.join(dir, 'vocab.json')).readAsString(),
      );
